Peer Assignment02
======================================
Created by Divya Saini on May 25, 2015

## Analysis of Severe Weather Events across United States

### Synonpsis  
This report analyzes the severe weather events across United States and primarily addresses following two questions based on the storm database collected from the U.S. National Oceanic and Atmospheric Administration's (NOAA) from 1950 - 2011.Firstly, which types of events are most harmful with respect to population health? And second, which types of events have the greatest economic consequences? We will use the estimates of fatalities, injuries, property and crop damage to decide which types of event are most harmful to the population health and economy. From these data, we found that excessive heat and tornado are most harmful with respect to population health, while flood, drought, and hurricane/typhoon have the greatest economic consequences.

### Basic settings
```{r}
echo = TRUE  # To make code visible
...
### Data Processing
Data file is downloaded and unzipped.
```{r}
setwd("C:/Users/Administrator/Desktop/CourseraWD")
...
```
Read the csv file. If the data already exists in the working environment, no need to load it again, else read the csv file.
```{r}
if (!"stormData" %in% ls()) {
...
Now we will find the number of **fatalities** and **injuries** that are caused by the severe weather events. We will fetch the first 15 most severe types of weather events.
```{r}
...
#### Impact on Economy
Now convert the **property damage** and **crop damage** data into comparable numerical forms according to the meaning of units described in the code book ([Storm Events](http://ire.org/nicar/database-library/databases/storm-events/)). Both `PROPDMGEXP` and `CROPDMGEXP` columns record a multiplier for each observation where we have Hundred (H), Thousand (K), Million (M) and Billion (B).
